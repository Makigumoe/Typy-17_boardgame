...
电子侦察的判定为：ER（自身的）+ES（目标的）+d20 vs. 15

此外，进行侦察还需要满足一些额外条件：
* 进行光学侦察时，OR必须大于0，而且自身到目标的 [视线](#245-视线判定) 成立。
* 进行电子侦察时，ER和ES必须大于0。

侦察判定成功之后，会根据差值决定获得几级情报值。侦察判定成功时，至少获得1级情报值；每当差值比1大4，就额外获得1级情报值。举例，在一次成功的侦察检定中差值为5，那么获得2级情报值；若差值为4，则只获得1级情报值。此外，一些地形会限制能获得的最大情报值等级。
